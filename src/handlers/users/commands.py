...
@dp.message_handler(text=['Привет', 'Начать', 'привет'])
@dp.message_handler(commands='start')
async def answer_start_command(message: types.Message):
    await message.answer(text=f'Привет, {hbold(message.from_user.username)}!'
                              f'\nРад тебя видеть!'
                              f'\nЯ - {hitalic("Интернет-магазин")} {hstrikethrough("КОТИКИ")}🐱'
                              f'\nБудем вместе делать {hunderline("покупки")}🐾'
                              f'\nА это мой создатель, пиши ему, если что: {hlink(url=r"https://t.me/kori_si", title="тык")}',
...
async def answer_menu_command(message: types.Message, user_basket: tuple[int | str]):
    await message.answer(text='Тестируем middlewares'
                         f'\nВот, что нам пришло из него:{user_basket}')
